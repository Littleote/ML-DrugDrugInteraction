...
class deptree:
    ...
    def get_subtree_offset_span(self, n):
        ...
        if children:
            children_left, children_right = self.get_subtree_offset_span(children[0])
            left = min(left, children_left)
            children_left, children_right = self.get_subtree_offset_span(children[-1])
            right = max(right, children_right)
        return left, right
